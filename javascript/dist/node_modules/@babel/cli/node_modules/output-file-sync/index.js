...
const nameMessageMap = new Map([['mode', `Expected \`mode\` ${COMMON_MESSAGE} file and directory mode`], ['fileMode', `Expected \`fileMode\` ${COMMON_MESSAGE} file mode`], ['dirMode', `Expected \`dirMode\` ${COMMON_MESSAGE} directory mode`]]);
...
module.exports = function outputFileSync(filePath, data, options) {
  ...
  if (typeof data !== 'string' && !Buffer.isBuffer(data) && !(data instanceof Uint8Array)) {
    throw new TypeError(`Expected file content to be a string, Buffer or Uint8Array, but got ${inspect(data)} instead.`);
  }
  ...
  if (options !== null && options !== undefined) {
    if (typeof options === 'string') {
      ...
      throw new TypeError('Expected a string to specify file encoding or ' + `an object to specify output-file-sync options, but got ${inspect(options)}.`);
    }
  ...
  const createdDirPath = mkdirpSync(dirname(filePath), options.dirMode !== undefined ? Object.assign({
    fs
  }, options, {
